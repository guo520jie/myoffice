...
function importVideo(){
	...
	video.add=function(options){
		if(options && options.src && options.shell){
			var defaults = {controls:true,autoplay:true,playsinline:true,autosize:'fill',muted:false};
			var opts = $.extend(defaults,options);
			var ht=window.innerWidth*9/16;
			var top=window.innerHeight/2-ht/2;
			opts.shell.css({width:window.innerWidth,height:ht,top:top});
			var container=$('<video></video>').attr({src:opts.src}).css({width:window.innerWidth,height:ht}).appendTo(opts.shell);
			if(opts.poster) container.attr({poster:opts.poster});
			if(opts.playsinline) container.attr({'playsinline':'','webkit-playsinline':'','x5-video-player-type':'h5-page'});
			else container.attr({'x5-video-player-type':'h5'});
			if(opts.muted) container[0].muted=true;
	...
	function video_play(e){
		...
		if(vid && vid!=''){
			...
			if(type=='youku') $('<iframe width=100% height='+ht+' src=http://player.youku.com/embed/'+vid+ (autoplay?'?autoplay=true':'') + ' frameborder=0 allowfullscreen></iframe>').css({top:top}).appendTo(box);
				...
				var container=$('<video playsinline webkit-playsinline x5-video-player-type="h5-page" controls></video>').attr({src:vid,poster:$(this).data('poster')}).css({width:window.innerWidth,height:ht,top:top}).appendTo(box);
