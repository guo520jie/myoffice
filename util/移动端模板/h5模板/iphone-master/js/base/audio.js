		...
		if(list.length>0){
			...
			for(var i=0; i<list.length; i++){
				...
				if(opts.src!=''){
					...
					if(this.webAudio) this.soundList.push(new webAudio(option));
					else this.soundList.push(new localAudio(option));
				}//edn if
			}//end for
			this.soundMax=this.soundList.length;
...
//  	console.log(get_src(this.src)+' pause');
	...
	audio.bgm=function(options){
		var defaults = {src:'',btn:$('a.bgmBtn'),webAudio:false,autoplay:true};
	...
	localAudioBgm.prototype.play=function(e){
		...
		if(!_this.played && _this.loaded){
			...
			if(_this.btn.length>0){
				_this.btn.one('click',{target:_this},_this.pause);
				_this.icon.removeClass().addClass('play');
			}//edn if
		}//edn if
	}//end func
	...
	localAudioBgm.prototype.pause=function(e){
		...
		if(_this.btn.length>0){
			_this.btn.one('click',{target:_this},_this.play);
			_this.icon.removeClass().addClass('pause');
		}//edn if
	}//end func
...
function importBgm(){
	...
	bgm.init=function(options){
		...
		if(opts.src!=''){
			this.btn=$('<a class="bgmBtn"><i></i></a>').appendTo('article');
			this.icon=this.btn.children();
	...
	bgm.show=function(resume=true){
		if(resume) this.play();
		this.btn.show();
	}//edn func
	
	bgm.mute=function(muted=false){
