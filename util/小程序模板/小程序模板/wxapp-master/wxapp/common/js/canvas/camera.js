 ...
 */
var Camera = function (id, options) {
    var s = this;
     ...
     * 选择图片
     */
    s.chooseImage = function (imgObj, data, succ = function () { }, fail = function () { }, sizeType = ['original', 'compressed'], sourceType = ['album', 'camera']) {
        if (imgObj) {
            imgFileObj = imgObj;
            _parseSize();
            _updateCanvas();
            ...
            wx.chooseImage({
                ...
                success: (res) => {
                    ...
                    if (tempFilePaths.length > 0) {
                        console.log('wx chooseImage succ');
                        succ();
                        //计算一下图片
                        wx.getImageInfo({
                            src: tempFilePaths[0],
